  ...
  <div class="header">
    ...
    <div class="user-info">
      <el-badge :value="3" type="primary">
        <el-button type="text" size="large"><span class="icon">🔔</span></el-button>
      </el-badge>
      <el-dropdown @command="changeLang">
        <span class="lang-select">{{ locale === 'en' ? 'English' : '中文' }}</span>
        <template #dropdown>
          <el-dropdown-menu>
      ...
      <el-dropdown>
        ...
        <template #dropdown>
          <el-dropdown-menu>
            <el-dropdown-item>{{ t('header.personalSettings') }}</el-dropdown-item>
            <el-dropdown-item @click="store.logout(); $router.push('/login')">{{ t('header.logout') }}</el-dropdown-item>
          </el-dropdown-menu>
        </template>
      </el-dropdown>
    </div>
